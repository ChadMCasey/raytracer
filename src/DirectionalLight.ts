...
export default class DirectionalLight extends Light {
  ...
  computeIllumination(P: Vec3, N: Vec3, V: Vec3, s: number): number {
    ...
    const specularScalar: number = this.computeScalarHighlight(N, V, s, this.direction, DotNL);
  ...
  computeScalarHighlight(N: Vec3, V: Vec3, s: number, L: Vec3, DotNL: number): number {
    if (s === -1) return -1; 
